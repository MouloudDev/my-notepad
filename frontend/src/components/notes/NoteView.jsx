...
function NoNote() {
  return (
    ...
    <p className="text-lg text-center dark:text-gray-200 max-w-xl mx-auto">
      <img src="./Empty-amico.png" alt="illustration" />
...
export default function NoteView() {
  ...
  return (
    <div className="w-full px-4">
      <span className="block text-md font-semibold dark:text-white">
        Created at: <span className="text-sm text-gray-500 dark:text-gray-300">{formatDateTime(createdAt)}</span>
      </span>
      <span className="block text-md font-semibold dark:text-white">
        Updated at: <span className="text-sm text-gray-500 dark:text-gray-300">{formatDateTime(updatedAt)}</span>
      ...
      <input
        value={title}
        onChange={(e) => setTitle(e.target.value)}
        className="w-full text-3xl border border-gray-400 text-left font-bold my-2 p-3 rounded-lg outline-none dark:text-white dark:bg-slate-900"
      />
